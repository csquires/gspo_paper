...
from evaluate import get_precision_skeletons, get_tprs_skeletons
...
if FCI:
    ...
    for nsamples in nsamples_list:
        ...
        precision = np.array([
            get_precision_skeletons(ngraphs, nnodes, nlatent, exp_nbrs, nsamples, alg, **kwargs)
            for kwargs in alg2kwargs[alg]
        ])
        ...
        marked_points = avg_precision[marked_point_ixs], avg_tprs[marked_point_ixs]
        plt.scatter(*marked_points, color=algs2colors[alg], marker='*', s=STAR_SIZE)

        max_fpr = max(max_fpr, *avg_precision)
        sort_ixs = np.argsort(avg_precision)
        plt.plot(
            avg_precision[sort_ixs],
            avg_tprs[sort_ixs],
...
if GSPO_EMPTY:
    ...
    for nsamples in nsamples_list:
        ...
        precision = np.array([
            get_precision_skeletons(ngraphs, nnodes, nlatent, exp_nbrs, nsamples, alg, initial=initial, **kwargs)
            for kwargs in alg2kwargs[alg]
        ])
        avg_tprs = np.mean(tprs, axis=1)
        avg_precision = np.mean(precision, axis=1)
...
#         for kwargs in alg2kwargs[alg]
#     ])
#     print(alg, tprs.mean(axis=1), precision.mean(axis=1))
#     avg_tprs = np.mean(tprs, axis=1)
#     avg_precision = np.mean(precision, axis=1)
#     max_fpr = max(max_fpr, *avg_precision)
#     sort_ixs = np.argsort(avg_precision)
#     plt.plot(avg_precision[sort_ixs], avg_tprs[sort_ixs], color=nsamples2color[nsamples], linestyle=algs2linestyles[alg], marker='.')

FINAL = True
if not FINAL:
    plt.title(f"nnodes={nnodes},exp_nbrs={exp_nbrs},nlatent={nlatent},ngraphs={ngraphs}")
plt.plot([0, 1], [1, 0], alpha=.5, color='grey')
plt.xlabel('Average Precision (Skeleton)')
plt.ylabel('Average Recall (Skeleton)')
plt.legend(handles=alg_handles+nsample_handles)
plt.tight_layout(pad=.3)
plt.savefig(get_fig_name_vary_samples(ngraphs, nnodes, nlatent, exp_nbrs, 'precision_recall'))
